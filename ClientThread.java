...
public class ClientThread extends Thread {
	...
	private void GetSensorInfoByType(byte[] DecPacket, int p,
		...
		switch (DecPacket[p]) {
			case (byte) 0x0b: // SHT10温湿度传感器(高精度)
				...
				if(Humidity > 30)
				{
					node.setDryerState((byte) 1);
				}
				else
				{
					node.setDryerState((byte) 0);
				}
			...
			case (byte) 0x11:
				...
				StrSENSOR_DATA = "振幅" + ((int) (((DecPacket[i] & 0xff) << 8)
						+ ((DecPacket[i + 1]) & 0xff))) + "hz";
				if((((DecPacket[i] & 0xff) << 8) + ((DecPacket[i + 1]) & 0xff)) > 3)
				{
					node.setshake_turnto((byte) 1);//振幅大于20，触发警报
				}
				else
				{
					node.setshake_turnto((byte) 0);// 振幅小于20，显示安全
				}
			...
			case (byte) 0x13:
				...
				StrSENSOR_DATA = "光强："
						...
						+ "lx";
				;
				if((((DecPacket[i] & 0xff) << 8) + ((DecPacket[i + 1]) & 0xff))>170)
				{
					node.setDengState((byte) 1); //光照强度大于170， 展览灯会自动关闭
				}
				else
				{
					node.setDengState((byte) 0);   //光照强度小于170，展览灯会自动开启
				}
				break;
            case (byte) 0x14:
                ...
                if ("0x00".equals(str)) {
					...
					node.setvoice_turnto((byte) 1);
                } else if ("0x01".equals(str)) {
                    StrSENSOR_DATA = "未检测到声音";
					node.setvoice_turnto((byte) 0);
                }
                break;
			default:
	...
	public void addANode(NodeInfo node) {
		...
		if (node.getType() == 0x11) {

			Message msg = new Message();
			msg.what = 0x2224;
			msg.obj = node;
			if (ControlInterface.handler != null
					&& ControlInterface.currentUiName.equals("振动传感器")) {
				if (ControlInterface.nodeinfo != null
						&& ControlInterface.nodeinfo.getId().equals(
						node.getId()))
					ControlInterface.handler.sendMessage(msg);
...
/*case (byte) 0x11: // 直流马达模块
			...
			if (DecPacket[12] == 0) {
				node.setvoice_turnto((byte) 1);
				node.setvoice_sudu(DecPacket[13]);
			} else if (DecPacket[13] == 0) {
				node.setvoice_turnto((byte) 0);
				node.setvoice_sudu(DecPacket[12]);
			}
			break;*//*
...
*/
/*
			if (!this.StrSENSOR_TYPE.equals(""))
			{
				//paramStringBuilder.append("\n" + this.StrSENSOR_TYPE);
				node.setNodeName(this.StrSENSOR_TYPE);
			}
			if (!this.StrSENSOR_DATA.equals(""))
			{
				//paramStringBuilder.append("\n" + this.StrSENSOR_DATA);
				node.setInfo(this.StrSENSOR_DATA);
				if (this.StrSENSOR_TYPE.equals("SHT10温湿度传感器(高精度)"))
					Log.i("ccccccccccccc", "xxxxxxxxxxxxxxxxxxxxxxx");
			}

			this.StrSENSOR_TYPE = "温湿度传感器";
			float f1 = (float)(-39.700000000000003D + 0.01D * (short)(((DecPacket[(i + 2)] & 0xFF) << 8) + (DecPacket[(i + 3)] & 0xFF)));
			i = (short)(((DecPacket[i] & 0xFF) << 8) + (DecPacket[(i + 1)] & 0xFF));
			float f2 = (float)(-2.0468D + 0.0367D * i + -1.5955E-006D * i * i);
			f2 = (float)((f1 - 25.0F) * (0.01D - 8.000000000000001E-005D * f2) + f2);
			node.setWendu(f1);
			node.setShidu(f2);
			this.StrSENSOR_DATA = ("温度:" + this.floatNum.format(f1) + "℃\n" + "湿度:" + this.floatNum.format(f2) + "%");
			Log.i("bbbbbbbbbbbbb", this.StrSENSOR_DATA);
			break;*//*
	...
	public void addANode(NodeInfo node) {
		...
		if (node.getType() == 0x0b) {
			...
			msg.obj = node;
			if (ControlInterface.handler != null
					&& ControlInterface.currentUiName.equals("温度湿度传感器"))
				ControlInterface.handler.sendMessage(msg);
